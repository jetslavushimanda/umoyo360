...
async function ensureClinicsSeeded() {
  ...
  if (snap.empty) {
    for (const clinic of CLINIC_SEED_DATA) await addDoc(collection(db, "clinics"), clinic);
  }
}

async function loadClinicsForDistrict(province, district) {
  showLoading("clinic-results");
  document.getElementById("location-display").textContent = `Showing clinics in: ${district}, ${province}`;
  try {
    const q = query(collection(db, "clinics"), where("district","==",district), where("province","==",province));
...
function setupLocationSelector() {
  document.getElementById("btn-change-location")?.addEventListener("click", () => document.getElementById("location-panel")?.classList.toggle("hidden"));
  ...
  document.getElementById("btn-search-district")?.addEventListener("click", async () => {
    const prov = provinceSelect.value, dist = districtSelect.value;
    if (!prov || !dist) return;
